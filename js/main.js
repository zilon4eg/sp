    ...
    for (i=0; i<data.dishes.length; i++) {
        ...
        if ((dishIsVisible) && (!dishIsDelete)) {
            ...
            if (sideMenuCategory.toLowerCase() === dishCategory.toLowerCase()) {
                ...
                dish.querySelector('.dish_item-img').src = `../img/dishes/${data.dishes[i].id}.jpeg`;
        ...
        el.onclick = function(event) {
            ...
            basket.forEach((item, index, array) => {
                if (item.id === Number(modalBasketItem.dataset.id)) {
                    ...
                    document.getElementsByTagName('html')[0].style.height = '100%';
                    correctSiteSize();
                    if (document.querySelectorAll('.modal_basket-item').length < 1) {
                        modalBasketWindow.classList.remove('modal_basket-visible');
                        const modalBasketItems = document.querySelectorAll('.modal_basket-item');
                        for (let item of modalBasketItems) {
                            item.remove();
    ...
    basket.forEach(el => {
        ...
        for (i=0; i<data.dishes.length; i++) {
            if (Number(data.dishes[i].id) === Number(el.id)) {
                basketForm.querySelector('.modal_basket-item-img img').src = `../img/dishes/${data.dishes[i].id}.jpeg`;
...
function correctSiteSize() {
    ...
    const siteSize = document.getElementsByTagName('html')[0].offsetHeight;
    if (Number(siteSize) < (Number(modWindowSize) + 100)) {
        document.getElementsByTagName('html')[0].style.height = `${Number(modWindowSize) + 100}px`;
    }
}
